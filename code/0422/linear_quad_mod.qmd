...
merged_long <- merged %>%
  pivot_longer(
    cols = -c(time, temp, vpd, co2, eppfd_g, eppfdi_g),
...
```{r}
mod_lm1 <- lm(qy ~ ppfd + eppfd + ppfdi + eppfdi + temp + vpd + co2, data = merged_long)
vif(mod_lm1)
```
## 2. PPFDs only
```{r}
...
```{r}
summary(mod_lm6)$coefficients
```

## Measured vs Predicted
```{r}
merged_long$pred_mod6 <- predict(mod_lm6)

ggplot(merged_long, aes(x = qy, y = pred_mod6)) +
...
# 1. ePPFD  only: Model evaluation
## Create a recipe
```{r}
lm_recipe <- recipe(qy ~ eppfd + temp + vpd + co2, data = merged_train)
```
...
lm_recipe_scaled <- recipe(qy ~ eppfd + temp + vpd + co2, data = merged_train) %>%
