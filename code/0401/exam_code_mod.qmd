...
```{r}
merged_long$pred_mod3 <- predict(mod_lm3)

ggplot(merged_long, aes(x = qy, y = pred_mod3)) +
...
# 2. ePPFDi only: Model evaluation
## Create a recipe
```{r}
lm_recipe_2 <- recipe(qy ~ eppfdi + temp + vpd + co2, data = merged_train)
```
...
```{r}
mod_quad <- lm(qy ~ eppfd + eppfdi + temp + I(temp^2) + vpd + co2, data = merged_long)
vif(mod_quad)
```
## Recipe and Workflow
```{r}
